...
Die Quality Regel prüft ob korrekte Daten im Vergleich mit CAIMAN in Darwin gespeichert sind. Trifft dies nicht zu, wird ein Dataissue generiert.

Um das Dataissue zu bereinigen müssen korrekte Daten die in CAIMAN enthalten sind manuell eingetragen werden oder man setzt das Feld "automatisierte Updates durch Schnittstellen zulassen" auf "Ja" wodurch die Daten automatisch aus CAIMAN gezogen werden.
...
sub qcheckRecord
{
   ...
   if ($rec->{email} ne "" && $rec->{cistatusid}<=5){
      ...
      if (!defined($caimanrec) || 
          (defined($caimanrec) &&                  # find maybe a new primary
           lc($caimanrec->{primary}) ne "true")){  # workrelation
         #####################################################################
         ...
         #####################################################################
         if (defined($caimanrec) && $caimanrec->{tcid} ne ""){ 
            my $dsid=$caimanrec->{tcid};
            $caiman->ResetFilter();
            $caiman->SetFilter({tcid=>$dsid,active=>\'true'});
            my @l=$caiman->getHashList(qw(primary active tcid 
                                          email email2 email3));
            my @emails=();
            my $foundPrimaryWR=0;
            foreach my $caimansubrec (@l){
               foreach my $emailattr (qw(email email2 email3)){
                  my $emailtyp="alternate";
                  if (lc($caimansubrec->{primary}) eq "true"){
                     $foundPrimaryWR++;
                  }
            ...
            if ($foundPrimaryWR==0 && $#emails>-1){ # if only on secondary
               msg(WARN,"no primary CAIMAN-Workrelation found for ".
                        "userid=$rec->{userid}");
               $emails[0]->{emailtyp}="primary";    # workrelations exists, then
            }                                       # make them primary
